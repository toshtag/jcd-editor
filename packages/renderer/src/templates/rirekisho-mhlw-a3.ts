...
const renderPhotoBox = (profilePhoto: ProfilePhoto | undefined): string => {
  ...
  return `<div class="jcd-mhlw__photo jcd-mhlw__photo--filled" style="${style}"><img class="jcd-mhlw__photo-image" src="${escapeHtml(source.dataUri)}" alt="${escapeHtml(altText)}"${imgStyle}></div>`;
};
