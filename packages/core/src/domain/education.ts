...
export const educationSchema = v.pipe(
  v.object({
    institutionName: v.optional(createNonBlankTextSchema(INSTITUTION_NAME_MAX, '学校名')),
    faculty: v.optional(createNonBlankTextSchema(FACULTY_MAX, '学部')),
    department: v.optional(createNonBlankTextSchema(DEPARTMENT_MAX, '学科・専攻')),
    degree: v.optional(createNonBlankTextSchema(DEGREE_MAX, '学位')),
    startDate: v.optional(isoYearMonthStringSchema),
    endDate: v.optional(isoYearMonthStringSchema),
    status: v.optional(createNonBlankTextSchema(STATUS_MAX, '在籍状況')),
