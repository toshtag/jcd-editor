...
**ステータス: 初期開発中。Core モデル、HTML レンダラー (履歴書 / 職務経歴書 + 印刷時改ページ CSS)、IndexedDB 保存、全 section 編集対応の local-web UI (基本情報 / 職歴 / 学歴 / スキル / 資格 / プロジェクト / 写真) + dirty state、JSON import/export、プロファイル削除 UI、CLI による JSON → PDF 生成 (Playwright local 完結) を実装済み。AI 連携、デスクトップ化、クラウド同期は未実装です。**
...
| [docs/cli.md](docs/cli.md) | CLI 利用ガイド (JSON → PDF) |
| [docs/data-format.md](docs/data-format.md) | CareerProfile JSON 形式の公開仕様 |
