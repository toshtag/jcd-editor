...
**ステータス: 初期開発中。Core モデル、HTML レンダラー (履歴書 / 職務経歴書 + 印刷時改ページ CSS)、IndexedDB 保存、全 section 編集対応の local-web UI (基本情報 / 職歴 / 学歴 / スキル / 資格 / プロジェクト)、JSON import/export、プロファイル削除 UI、ローカル PDF 生成基盤を実装済み。dirty state / 写真入力 / CLI PDF / AI 連携 / デスクトップ化 / クラウド同期は未実装です。**
