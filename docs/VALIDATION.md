...
## 4. UI validation (実装済み: 即時 feedback / 構造化表示 / accessibility 連携)

フィールド単位の即時フィードバック (例: 入力中に「メール形式が不正です」と表示) は **UX 層の責務** です。`apps/local-web` で実装済み。

### 原則 (継続)

- UI validation は core parsing を **置き換えない**
- 永続化 / インポート / CLI 経由の処理では、UI を通っていないルートを含めて core parse を必ず通す
- UI は UX の上塗りとしての即時性を提供する役割であり、データ整合性の最終ゲートではない

これにより、UI を経由しないインポートや別環境からのファイル読み込みでもデータ整合性が保たれます。

### 実装済みの UI validation 機構

`apps/local-web` で、`safeParseCareerProfile` の結果を以下の経路で user に提示する:

| 層 | 役割 | 実装 |
|----|------|------|
| validation summary (form 上部) | 全 issue を構造化 list で表示、各 item に jump button | `validation-summary.ts` (pure) + `main.ts` event delegation |
| inline error (input 直下) | 該当 input のすぐ下に message を表示、複数 issue は ` / ` 連結 | `main.ts` の `renderInlineError` (DOM 操作のみ) |
| 日本語 label 化 | `educationHistory.0.endDate` のような dot path を `学歴 1 件目 > 卒業・終了月` に翻訳 | `validation-labels.ts` (pure) |
| `aria-invalid` | invalid input に `true` を付ける | `validation-inputs.ts` (pure) + `main.ts` |
| `aria-describedby` | invalid input から該当 summary item に SR 経由でメッセージを伝える | `main.ts` の `markInputsInvalid` |
| dirty state | 未保存変更を indicator で表示、load / reload 前の confirm | `main.ts` の `isDirty` フラグ |

a11y regression は CI の axe-core test (`apps/local-web/src/__tests__/accessibility.test.ts`) で自動検出する。

詳細な設計判断は `docs/investigations/accessibility-audit.md` 参照。
...
| 4. UI validation | 実装済み (summary / inline / aria-* / dirty state / axe-core CI) | `apps/local-web` の `validation-*` モジュール + `main.ts` |
