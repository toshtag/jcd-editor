...
- `feat/pdf-port-foundation` (PR 番号は作成後反映): `packages/pdf` に `PdfPort` / `PdfRenderResult` / `PdfRenderMetadata` / `PdfError` を最小 surface で導入。実 PDF 生成 (Playwright / Puppeteer / pdf-lib 等) は **含まない**。`PdfPort` の shape は **provisional** であり、次 PR で実装目線の見直しを行い、後方互換しない変更を許容する。pdf → renderer の一方向依存。`packages/core` / `packages/renderer` / `packages/templates` / `docs/ARCHITECTURE.md` すべて無変更
